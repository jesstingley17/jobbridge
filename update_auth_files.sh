...
echo "📝 Updating auth and API files..."

# Read current api/index.ts to get the header
head -75 api/index.ts > /tmp/api_header.txt

# Create updated api/index.ts with env validation
cat > api/index.ts << 'APIEOF'
...
echo "✓ Updated api/index.ts"

git add api/index.ts
git commit -m "Add environment validation and clearer error handling to Vercel function

- Validate required Supabase env vars at startup (SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SESSION_SECRET)
- Add try/catch around initializeApp to surface clear errors instead of FUNCTION_INVOCATION_FAILED
- Import ensureEnvWarn and ensureEnvOrThrow helpers
- Fail fast in production, warn in development

This fixes the vague 500 errors by showing exactly which env vars are missing."

git push origin main

echo ""
echo "✅ API file updated and pushed!"
